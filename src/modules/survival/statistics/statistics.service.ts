...
export class StatisticsService {
  constructor(
    ...
    private readonly planUserRepo: Repository<SurvivalPlanUser>,
    @InjectRepository(SurvivalPlanServer)
    private readonly planServerRepo: Repository<SurvivalPlanServer>,
    @InjectRepository(SurvivalPlanSession)
    private readonly planSessionRepo: Repository<SurvivalPlanSession>,
    @InjectRepository(SurvivalPlanKill)
    private readonly planKillRepo: Repository<SurvivalPlanKill>,
    @InjectRepository(SurvivalPlanWorldTime)
    private readonly planWorldTimeRepo: Repository<SurvivalPlanWorldTime>,
    @InjectRepository(SurvivalPlanWorld)
    private readonly planWorldRepo: Repository<SurvivalPlanWorld>,
    @InjectRepository(SurvivalPlanTps)
    private readonly planTpsRepo: Repository<SurvivalPlanTps>,
    @InjectRepository(SurvivalPlanPing)
    private readonly planPingRepo: Repository<SurvivalPlanPing>,
    @InjectRepository(SurvivalPlanNickname)
    private readonly planNicknameRepo: Repository<SurvivalPlanNickname>,
    @InjectRepository(SurvivalPlanGeolocation)
    private readonly planGeolocationRepo: Repository<SurvivalPlanGeolocation>,
    @InjectRepository(SurvivalPlanUserInfo)
    private readonly planUserInfoRepo: Repository<SurvivalPlanUserInfo>,
  ) {}
  ...
  async getServerStats(dto: GetServerStatsDto) {
    try {
      ...
      const range = dto.range || 24; // Default to 24 hours
      const now = Math.floor(Date.now() / 1000);
      const startTime = now - range * 3600;

      // Debug log to check timestamps
      console.log(
        `Survival - Current timestamp: ${now}, Start time: ${startTime}`,
      );

      // For consistency with boxpvp, use a very old timestamp and filter in memory
      const veryOldTimestamp = 0; // Start from Unix epoch

      // Get TPS data for player activity chart (includes players_online)
      const tpsForActivity = await this.planTpsRepo.find({
        where: {
          server_id: server.id,
          date: MoreThanOrEqual(veryOldTimestamp),
        },
        order: { date: 'ASC' },
      });
      ...
      console.log(
        `Survival - TPS data count: ${tpsData.length}, Raw TPS for activity count: ${tpsForActivity.length}, Filtered count: ${filteredTpsForActivity.length}`,
      );
      if (filteredTpsForActivity.length > 0) {
        console.log(
          `Survival - Sample TPS data: date=${filteredTpsForActivity[0].date}, players_online=${filteredTpsForActivity[0].players_online}`,
        );
      }

      // Group TPS data by hour for the activity chart
      const activityByHour = new Map<number, number>();
      for (let hour = 0; hour < range; hour++) {
        const hourTimestamp = now - (range - hour) * 3600;
        activityByHour.set(hourTimestamp, 0);
      }

      // Use players_online from filtered TPS data
      filteredTpsForActivity.forEach((tpsData) => {
        // Ensure date is in seconds
        const tpsDateInSeconds =
        ...
        console.log(
          `Processing TPS entry for hour ${new Date(hourTimestamp * 1000).getHours()}, players=${tpsData.players_online}`,
        );

        // Find matching hour bucket
        for (const [bucketTime, count] of activityByHour.entries()) {
      ...
      const playerActivity = Array.from(activityByHour.entries()).map(
        ([time, count]) => {
      ...
      // Calculate averages from TPS data
      const latestTps = tpsData.length > 0 ? tpsData[0] : null;
      const tpsAvg =
        tpsData.reduce((sum, data) => sum + data.tps, 0) /
        (tpsData.length || 1);
      const cpuAvg =
        tpsData.reduce((sum, data) => sum + data.cpu_usage, 0) /
        (tpsData.length || 1);

      // Calculate uptime (this is an approximation since Plan doesn't store server uptime directly)
      // Ensure the date is in seconds (Plan might store it in milliseconds)
      const tpsDate = latestTps?.date
        ? latestTps.date > 10000000000
          ? Math.floor(latestTps.date / 1000)
          : latestTps.date
        : now;
      const uptimeHours = Math.max(0, Math.floor((now - tpsDate) / 3600));
      const uptimeDays = Math.floor(uptimeHours / 24);
      const uptimeRemainingHours = uptimeHours % 24;
      const uptime = `${uptimeDays} days, ${uptimeRemainingHours} hours`;
      ...
      HttpError({ code: 'SERVER_STATS_ERROR' });
      return {};
    }
  }
  ...
  async searchPlayers(query: string) {
    try {
      ...
      HttpError({ code: 'PLAYER_SEARCH_ERROR' });
      return [];
    }
  }
}
