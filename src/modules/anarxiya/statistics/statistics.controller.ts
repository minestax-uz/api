...
export class StatisticsController {
  ...
  async getFullServerStats() {
    return CoreApiResponse.success(
      await this.statisticsService.getFullServerStats('anarxiya'),
    );
  }
  ...
  async getServerStats(@Query('range', ParseIntPipe) range: number = 24) {
    return CoreApiResponse.success(
      await this.statisticsService.getServerStats('anarxiya', range),
    );
  }
  ...
  async getPlayerStats(
    ...
    return CoreApiResponse.success(
      await this.statisticsService.getPlayerStats('anarxiya', username, range),
    );
  }
}
