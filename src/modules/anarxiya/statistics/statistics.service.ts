...
export class StatisticsService {
  constructor(
    ...
    private readonly planUserRepo: Repository<AnarxiyaPlanUser>,
    @InjectRepository(AnarxiyaPlanServer)
    private readonly planServerRepo: Repository<AnarxiyaPlanServer>,
    @InjectRepository(AnarxiyaPlanSession)
    private readonly planSessionRepo: Repository<AnarxiyaPlanSession>,
    @InjectRepository(AnarxiyaPlanKill)
    private readonly planKillRepo: Repository<AnarxiyaPlanKill>,
    @InjectRepository(AnarxiyaPlanWorldTime)
    private readonly planWorldTimeRepo: Repository<AnarxiyaPlanWorldTime>,
    @InjectRepository(AnarxiyaPlanWorld)
    private readonly planWorldRepo: Repository<AnarxiyaPlanWorld>,
    @InjectRepository(AnarxiyaPlanTps)
    private readonly planTpsRepo: Repository<AnarxiyaPlanTps>,
    @InjectRepository(AnarxiyaPlanPing)
    private readonly planPingRepo: Repository<AnarxiyaPlanPing>,
    @InjectRepository(AnarxiyaPlanNickname)
    private readonly planNicknameRepo: Repository<AnarxiyaPlanNickname>,
    @InjectRepository(AnarxiyaPlanGeolocation)
    private readonly planGeolocationRepo: Repository<AnarxiyaPlanGeolocation>,
    @InjectRepository(AnarxiyaPlanUserInfo)
    private readonly planUserInfoRepo: Repository<AnarxiyaPlanUserInfo>,
  ) {}
  ...
  async getServerStats(dto: GetServerStatsDto) {
    try {
      ...
      console.log(
        `Anarxiya - Current timestamp: ${now}, Start time: ${startTime}`,
      );
      ...
      console.log(
        `Anarxiya - TPS data count: ${tpsData.length}, Raw TPS for activity count: ${tpsForActivity.length}, Filtered count: ${filteredTpsForActivity.length}`,
      );
      if (filteredTpsForActivity.length > 0) {
        console.log(
          `Anarxiya - Sample TPS data: date=${filteredTpsForActivity[0].date}, players_online=${filteredTpsForActivity[0].players_online}`,
        );
      }
